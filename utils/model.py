...
def train_model():
    ...
    model.fit(train_ds, epochs=60,callbacks=[tensorboard_callback])
    model.save("models/min_predict.keras")


def load_model():
    return tf.keras.saving.load_model("models/min_predict.keras")
...
def create_prediction_data(picked):
    now = datetime.datetime.now()
    mins = now.hour*60 + now.minute


    return {
        "tod":tf.convert_to_tensor([mins]),
        "dayofweek":tf.convert_to_tensor([now.weekday()]),
        "static_points":tf.convert_to_tensor([picked['static_points']]),
        "star":tf.convert_to_tensor([picked['star']]),
        "difficulty":tf.convert_to_tensor([picked['difficulty']]),
        "os":tf.convert_to_tensor([picked['os']]),
        "difficultyText":tf.convert_to_tensor([picked['difficultyText']]),
        "root_owns_count":tf.convert_to_tensor([picked['root_owns_count']]),
        "user_owns_count":tf.convert_to_tensor([picked['user_owns_count']]),
        "counterCake":tf.convert_to_tensor([picked['feedbackForChart']['counterCake']]),
        "counterTooEasy":tf.convert_to_tensor([picked['feedbackForChart']['counterTooEasy']]),
        "counterMedium":tf.convert_to_tensor([picked['feedbackForChart']['counterMedium']]),
        "counterBitHard":tf.convert_to_tensor([picked['feedbackForChart']['counterBitHard']]),
        "counterTooHard":tf.convert_to_tensor([picked['feedbackForChart']['counterTooHard']]),
        "counterExHard":tf.convert_to_tensor([picked['feedbackForChart']['counterExHard']]),
